    ...
    try:
        urllib.request.urlopen(
            urllib.request.Request(
                ...
                data=json.dumps({'email': 'ops.lead@ups.com', 'password': 'wrong_password'}).encode(),
        ...
        print(f'[PASS] Login Security Check: Correctly rejected {e.code}')

    # Test valid password login & dataset ingestion
    req = urllib.request.Request(
        'http://127.0.0.1:8000/api/auth/login-ingest/',
        data=json.dumps({'email': 'ops.lead@ups.com', 'organization_name': 'UPS Global Logistics', 'password': 'oasis2026'}).encode(),
        headers={'Content-Type': 'application/json'}
    )
...
try:
    ...
    print(f"       Use Case: {rec_data['project_meta']['use_case_id']} | SPOC: {rec_data['project_meta']['spoc']}")
    print(f"       Manpower Requirements: {rec_data['manpower_requirements']['normal_operations']['total_required_staff']} required staff, Status = {rec_data['manpower_requirements']['normal_operations']['status']}")
