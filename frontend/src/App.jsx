  ...
  useEffect(() => {
    api.get('/warehouses/')
      .then((res) => {
        ...
        if (!activeWarehouse && whs.length > 0) {
          const saved = localStorage.getItem('oasis_active_warehouse');
          if (saved) {
            try {
              const parsed = JSON.parse(saved);
              const found = whs.find((w) => w.warehouse_id === parsed.warehouse_id);
              setActiveWarehouse(found || whs[0]);
            } catch (e) {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${
      ...
      <header className={`sticky top-0 z-30 glass-card backdrop-blur-xl border-b px-4 sm:px-8 py-3.5 flex items-center justify-between ${
        isDark ? 'bg-stone-950/85 border-stone-800/80' : 'bg-white/95 border-[#E2E8F0] shadow-sm'
      }`}>
        ...
        {/* Global Active Facility Selector */}
        {availableWarehouses.length > 0 && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-stone-900/80 border border-stone-800 text-xs">
            ...
            <span className="text-stone-400 hidden md:inline">Facility:</span>
            <select
              value={activeWarehouse?.warehouse_id || ''}
              onChange={(e) => {
                const wh = availableWarehouses.find((w) => w.warehouse_id === e.target.value);
                if (wh) handleSwitchWarehouse(wh);
              }}
              className="bg-transparent text-yellow-400 font-bold outline-none cursor-pointer text-xs"
            >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-8 pt-6">
        ...
        {activeTab === 'simulator' && <Simulator activeWarehouse={activeWarehouse} />}
        {activeTab === 'recommendations' && <Recommendations activeWarehouse={activeWarehouse} />}
