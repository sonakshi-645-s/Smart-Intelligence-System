...
const VoiceAssistant = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        ...
        speechAccumulatorRef.current = currentTranscript;
        setInputText(currentTranscript);

        // Wait for 1.8s of silence after speech before executing full command
        if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
        silenceTimerRef.current = setTimeout(() => {
          if (speechAccumulatorRef.current && speechAccumulatorRef.current.trim().length > 3) {
            try {
              recognition.stop();
            } catch (e) {}
            setIsListening(false);
            const fullQuery = speechAccumulatorRef.current.trim();
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      setIsListening(false);
      if (speechAccumulatorRef.current && speechAccumulatorRef.current.trim().length > 2) {
        const query = speechAccumulatorRef.current.trim();
        speechAccumulatorRef.current = '';
